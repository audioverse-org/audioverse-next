...
fragment cardCollection on Collection{__typename id canonicalPath(useFuturePath:true)collectionContentType:contentType title startDate endDate duration image{id url(size:240 cropMode:DEFAULT)}allSequences:sequences{aggregate{count}}allRecordings:recordings(sequenceId:0){aggregate{count}}}
`;
export const CardSponsorFragmentDoc = `
fragment cardSponsor on Sponsor{__typename id title canonicalPath(useFuturePath:true)image{url(size:128)}collections{aggregate{count}}sequences{aggregate{count}}recordings{aggregate{count}}}
`;
export const CardPersonFragmentDoc = `
fragment cardPerson on Person{__typename id name canonicalPath(useFuturePath:true)image{id url(size:128)}recordings(first:2 orderBy:[{field:PUBLISHED_AT direction:DESC}]){aggregate{count}}}
`;
...
fragment sponsorListEntry on Sponsor{canonicalPath(useFuturePath:true)title image{url(size:128)}}
`;
...
query getSearchRecordings($language:Language!$term:String!$first:Int!$after:String){sermons(language:$language search:$term first:$first after:$after){aggregate{count}nodes{...cardRecording}pageInfo{hasNextPage endCursor}}}
${CardRecordingFragmentDoc}
${CardRecordingSequenceHatFragmentDoc}
${PersonLockupFragmentDoc}
${CardHatSponsorFragmentDoc}
...
query getSponsorListLetterPageData($language:Language!$startsWith:String){sponsors(language:$language startsWith:$startsWith first:1500 orderBy:[{field:TITLE direction:ASC}]){nodes{...sponsorListEntry}}sponsorLetterCounts(language:$language){letter count}}
${SponsorListEntryFragmentDoc}`;
